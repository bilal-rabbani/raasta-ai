...
def llm_router(prompt: str) -> dict:
    ...
    while f"GROQ_API_KEY_{i}" in st.secrets:
        groq_keys.append(st.secrets[f"GROQ_API_KEY_{i}"])
        i += 1
...
def web_search_agent(query: str, num_results: int = 5) -> list:
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "include_domains": ["gov.pk", "nadra.gov.pk", "fbr.gov.pk", "secp.gov.pk", "pec.org.pk"],
            },
            timeout=15,
        )
        response.raise_for_status()
        ...
        print(f"[RAASTA][web_search_error] {e}")
        return []


def extract_requirements_from_search(query: str, search_results: list) -> dict:
    if not search_results:
        return {"success": False, "text": None, "sources": []}
    ...
    prompt = f"""User's goal: {query}

Below are excerpts from official Pakistani government websites.
Summarize ONLY the concrete registration/document/process steps that are
explicitly mentioned in the text below, in 3-5 sentences.
Do not add anything not present in the text. If nothing relevant is found, say so clearly.

{sources_text}
"""
    result = llm_router(prompt)
    result["sources"] = [{"title": r.get("title", r["url"]), "url": r["url"]} for r in search_results]
    return result
...
def orchestrator(user_goal: str, business_structure: str = None) -> dict:
    ...
    state["web_fallback"] = None
    if not state["evidence"]:
        search_results = web_search_agent(user_goal)
        state["web_fallback"] = extract_requirements_from_search(user_goal, search_results)
        ...
        if not goal.strip():
            ...
            if not preview_profile["in_scope"]:
                ...
                if "business_structure" in preview_profile["missing"] and not st.session_state.business_structure:
                    ...
                    # ---------- Web fallback (unverified, live search) ----------
                    if state.get("web_fallback") and state["web_fallback"].get("success"):
                        st.warning("⚠️ Not found in our verified database — this answer was generated from a live web search and has NOT been manually verified. Please double-check with the official source before relying on it.")
                        st.info(state["web_fallback"]["text"])
                        st.caption("Sources:")
                        for src in state["web_fallback"].get("sources", []):
                            st.caption(f"- [{src['title']}]({src['url']})")
                        st.divider()
